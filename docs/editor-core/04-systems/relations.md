...
1. **Parametric placement.** The window's position is stored as
   *"on wall #42, at t=0.4 along its axis, sill offset 0.9m"* — never as absolute
...
2. **The host is modified by its attachments.** A window cuts an opening: the wall's
   *effective* geometry = base geometry − openings contributed by hosted entities
...
- `relations/` (Layer 2) maintains a **one-directional dependency DAG**: window depends
  on wall; wall never depends on window *dependency-wise* (effective geometry is computed
...
*"Place a window every 3 meters along the north wall"* is only a sane agent command if
