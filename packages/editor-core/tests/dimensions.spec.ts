...
describe('DimensionEntity — value is derived, never stored', () => {
  ...
  test('each wall contributes half its own thickness (the assembly rule)', () => {
    ...
    const axisId = session.dispatch<EntityId>('DIM.ADD', {
      wallA: a,
      wallB: b,
    });
    expect((session.doc.get(axisId) as DimensionEntity).getValue()).toBeCloseTo(
      4,
      9,
    );
  });
  ...
  test('DIM.ADD validates its two modes', () => {
    ...
    expect(() =>
      session.dispatch('DIM.ADD', { wallA: wall, wallB: wall, sideA: 'inner' }),
    ).toThrow();
    expect(() =>
      session.dispatch('DIM.ADD', {
        a: point(0, 0),
        b: point(1, 0),
        wallA: wall,
        wallB: wall,
...
describe('DIM.AUTO — inner clear widths, outer overall extents', () => {
  test('room + partition: inner dims per room, outer extents, regenerated on re-run', () => {
    ...
    const first = session.dispatch<{ removed: number; created: number }>(
      'DIM.AUTO',
      {},
    );
    ...
    expect(values.map((v) => Number(v.toFixed(2)))).toEqual([
      1.7, 3.7, 3.7, 3.7, 4.3, 6.3,
    ]);
    ...
    expect(
      session.doc.all().filter((e) => e instanceof DimensionEntity).length,
    ).toBe(6);
  ...
  test('auto dimensions land on the requested level; hand-placed dims survive', () => {
    ...
    drawRoom(session);
    const manual = session.dispatch<EntityId>('DIM.ADD', {
      a: point(0, 0),
      b: point(6, 0),
    });
