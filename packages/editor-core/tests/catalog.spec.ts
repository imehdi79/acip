...
function seedWallCatalog(session: EditorSession): {
  block: MaterialId;
  typeId: TypeId;
} {
...
describe('ENTITY.SETTYPE — the value-engineering primitive', () => {
  test('retypes entities, validates targetType, clears back to local props', () => {
    ...
    const slabType = session.dispatch<TypeId>('TYPE.ADD', {
      targetType: 'slab',
      name: 'S200',
    });

    const a = session.dispatch<EntityId>('WALL.ADD', {
      a: point(0, 0),
      b: point(6, 0),
      typeId,
    });
    const b = session.dispatch<EntityId>('WALL.ADD', {
      a: point(0, 2),
      b: point(6, 2),
      typeId,
    });
    ...
    const count = session.dispatch<number>('ENTITY.SETTYPE', {
      ids: [a, b],
      typeId: studType,
    });
    expect(count).toBe(2);
    ...
    expect(() =>
      session.dispatch('ENTITY.SETTYPE', { ids: [a], typeId: slabType }),
    ).toThrow();
...
describe('catalog removal guards', () => {
  ...
  test('MATERIAL.UPDATE renames and re-codes; quantities follow', () => {
    ...
    const { block, typeId } = seedWallCatalog(session);
    session.dispatch('WALL.ADD', { a: point(0, 0), b: point(10, 0), typeId });
    session.dispatch('MATERIAL.UPDATE', {
      id: block,
      name: 'AAC block',
      costCode: 'aac',
    });
