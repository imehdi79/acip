...
export class RequestsController {
  ...
  create(@Body() body: CreateRequestBody) {
    ...
    return this.prisma.request.create({
      data: {
        kind: body.kind === 'missing-price' ? 'missing-price' : 'missing-feature',
        text,
  ...
  update(@Param('id') id: string, @Body() body: { status?: unknown }) {
    ...
    if (!status) throw new HttpException("status must be 'open' or 'done'", 400);
    return this.prisma.request.update({ where: { id: Number(id) }, data: { status } });
  }
}
